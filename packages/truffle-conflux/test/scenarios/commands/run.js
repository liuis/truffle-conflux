...
describe("truffle-conflux run [ @standalone ]", () => {
  describe("when run without arguments", () => {
    ...
    it("displays general help", done => {
      CommandRunner.run("run", config, () => {
        ...
        assert(output.includes("Usage:        truffle-conflux run [<command>]"));
        done();
      });
    }).timeout(20000);
  ...
  describe("when run with an argument", () => {
    ...
    describe("with plugins configured", () => {
      describe("error handling", () => {
        ...
        it("throws error when plugins are missing truffle-conflux-plugin.json", done => {
          loadSandboxLogger(
            "../../sources/run/mockProjectWithMissingPluginConfig"
          ).then(() =>
            CommandRunner.run("run mock", config, () => {
              const output = logger.contents();
              assert(output.includes("Error: truffle-conflux-plugin.json not found"));
              done();
            })
          );
        }).timeout(10000);
        ...
        it("throws error if command in truffle-conflux-plugin.json uses an absolute path", done => {
          loadSandboxLogger(
      ...
      describe("when plugins configured and installed that support a given argument", () => {
        ...
        it("runs the command script", done => {
          CommandRunner.run("run mock", config, () => {
            const output = logger.contents();
            assert(output.includes("Running truffle-conflux-mock!"));
            done();
          });
        }).timeout(20000);
      });
