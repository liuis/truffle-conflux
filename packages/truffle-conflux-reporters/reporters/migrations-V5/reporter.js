...
class Reporter {
  ...
  async processDeploymentError(data) {
    ...
    switch (type) {
      ...
      // `Insufficient funds`
      case "ETH":
        const balance = await data.contract.web3.cfx.getBalance(data.from);
  ...
  async postDeploy(data) {
    let message;
    if (data.deployed) {
      const tx = await data.contract.web3.cfx.getTransaction(
        data.receipt.transactionHash
      );

      const block = await data.contract.web3.cfx.getBlock(
      ...
      const balance = await data.contract.web3.cfx.getBalance(tx.from);
