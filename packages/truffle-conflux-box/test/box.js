...
describe("truffle-conflux-box Box", () => {
  ...
  describe(".unbox()", () => {
    ...
    it("unboxes truffle box from github", done => {
      Box.unbox(TRUFFLE_BOX_DEFAULT, destination).then(truffleConfig => {
        ...
        assert(
          fse.existsSync(path.join(destination, "truffle-conflux-config.js")),
    ...
    it("does not copy the config files or ignored files in the config", () => {
      ...
      assert(
        fse.existsSync(path.join(destination, "truffle-conflux-init.json")) === false,
  ...
  describe("--force", () => {
    ...
    it("overwrites redundant files if init/unbox force flag used", done => {
      const truffleConfigPath = path.join(destination, "truffle-conflux-config.js");

      // preconditions
      fse.writeFileSync(
        truffleConfigPath,
        "this truffle-conflux-config.js file is different than the default box file",
        "utf8"
      );
      assert(
        fse.existsSync(truffleConfigPath),
        "mock truffle-conflux-config.js wasn't created!"
      );
      ...
      Box.unbox(TRUFFLE_BOX_DEFAULT, destination, { force: true }).then(() => {
        assert(
          fse.existsSync(truffleConfigPath),
          "truffle-conflux-config.js wasn't recreated!"
        );
        const newConfig = fse.readFileSync(truffleConfigPath, "utf8");
        assert(
          newConfig !== mockConfig,
          "truffle-conflux-config.js wasn't overwritten!"
        );
        done();
      });
    });
