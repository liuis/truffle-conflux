...
const TruffleError = require("truffle-conflux-error");
...
describe("plugin loader", () => {
  ...
  describe("checkPluginModules", () => {
    ...
    it("returns array of locally or globally installed options.plugins", () => {
      ...
      assert(
        pluginLoader.checkPluginModules({
          plugins: ["truffle-conflux-box", "truffle-conflux-config"],
          working_directory: process.cwd()
        })
      );
  ...
  describe("loadPluginModules", () => {
    it("throws when plugins are installed without a truffle-plugin.json configuration file", () => {
      assert.throws(
        () => {
          pluginLoader.loadPluginModules(["truffle-conflux-box"]);
        },
        TruffleError,
        "TruffleError not thrown!"
      );
      assert.throws(
        () => {
          pluginLoader.loadPluginModules(["truffle-conflux-box", "truffle-conflux-config"]);
        },
        TruffleError,
        "TruffleError not thrown!"
      );
